    ...
    if (var->notConst())         // 非常量
    {
        ...
        else                                       // 所有定义的变量和数组
        {
            ...
            if (isGlb)                     // 全局符号
            {
                ...
                if (isVar)
                    ldr_base(rsReg, rsReg, 0, isChar); // ldr r8,[r8]
            }
            else // 局部符号
            {
                if (isVar)
                    ldr_base(rsReg, "fp", off, isChar); // ldr r8,[fp,#-16]
                else
                    leaStack(rsReg, off); // add r8,fp,#-16
            }
        }
    }
    else // 常量
...
{
    emit("cmp", reg1, "#0");    // r8=0
    emit("moveq", rsReg, "#1"); // 如果相等则 rsReg = 1
    emit("movne", rsReg, "#0"); // 如果不相等则 rsReg = 0
}
