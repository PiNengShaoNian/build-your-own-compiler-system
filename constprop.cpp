...
ConstPropagation::ConstPropagation(DFG *g, SymTab *t, vector<Var *> &paraVar) : dfg(g), tab(t)
{
    ...
    for (int i = 0; i < glbVars.size(); ++i)
    {
        ...
        if (!var->isBase()) // 非基本类型
            val = NAC;
        else if (!var->unInit()) // 初始化的基本类型变量为const
            val = var->getVal();
    ...
    for (int i = 0; i < dfg->codeList.size(); ++i)
    {
    ...
    while (index--)
        initVals.push_back(UNDEF); // 添加初始值,初始化index（变量个数）个UNDEF
}
...
{
    if (left == NAC || right == NAC) // 与NAC交并结果必是NAC
        return NAC;
    else if (left == UNDEF) // 与UNDEF交并结果不变
        return right;
    else if (right == UNDEF)
        return left;
    else if (left == right) // 同值元素交并结果相同
        return left;
    else
        return NAC; // 异值元素交并结果是NAC
}
...
void ConstPropagation::join(Block *block)
{
    ...
    int prevCount = prevs.size();        // 前驱个数
                                         // for之前的判断是优化计算 ，可以删除
    if (prevCount == 1)                  // 唯一前驱，取出前驱集合初始化
    {
        in = prevs.front()->outVals;
        return;
    }
    if (prevCount == 0) // 没有前驱，直接初始化in集合
    {
        in = initVals;
        return;
    }
...
void ConstPropagation::translate(InterInst *inst, vector<double> &in, vector<double> &out)
{
    ...
    if (inst->isExpr())              // 基本运算表达式x=?，计算新发现的传播值
    {
        ...
        else if (op >= OP_ADD && op <= OP_OR) // 二元运算x=y+z
        {
            ...
            { // 参数不在值列表，必是常量
                ...
                lp = in[arg1->index]; // 左操作数
            if (arg2->index == -1)
            { // 参数不在值列表，必是常量
                if (arg2->isBase())
                    rp = arg2->getVal(); // 初始化的基本类型变量为const
            }
            else
                rp = in[arg2->index]; // 右操作数
    ...
    else if (op == OP_SET || op == OP_ARG && !arg1->isBase())
    {
        ...
        for (int i = 0; i < out.size(); ++i) // out全部置为NAC
            out[i] = NAC;
    }
    else if (op == OP_PROC) // 破坏运算call f()
    {
        for (int i = 0; i < glbVars.size(); ++i)
            out[glbVars[i]->index] = NAC; // 全局变量全部置为NAC
    }
    else if (op == OP_CALL)
    {
            ...
            if (inst->isExpr())          // 处理每一个可能常量传播的指令
            {
                ...
                else if (op >= OP_ADD && op <= OP_OR && !(op == OP_AS || op == OP_NEG || op == OP_NOT))
                {
                    ...
                    if (arg1->index == -1) // 参数不在值列表，必是常量
                    {
                        ...
                        lp = inst->inVals[arg1->index]; // 左操作数
                    if (arg2->index == -1)              // 参数不在值列表，必是常量
                    {
                        ...
                        rp = inst->inVals[arg2->index]; // 右操作数
                    ...
                    if (op == OP_ADD)       // z=0+y z=x+0
                    {
                        ...
                        if (dor && right == 0)
                        {
                            newArg1 = arg1; /*if(arg2->index==-1)delete arg2;*/
                        }
                    }
             ...
             i != dfg->blocks[j]->insts.end(); i = k)
        {
            ...
            if (inst->isJcond()) // 处理每一个可能常量传播的条件跳转指令，无条件跳转指令不用关心
            {
                ...
                if (arg1->index == -1)              // 参数不在值列表，必是常量
                {
                    ...
                    cond = inst->inVals[arg1->index]; // 条件值
                ...
                if (op == OP_JT) // 条件真跳转
                {
                    if (cond == 0) // 条件不满足,一定会fallthrough
                    {
