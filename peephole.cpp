...
{
    int i = 0;
    for (pos = code.begin(); pos != code.end() && i < size; pos++, i++)
    {
        // 还有指令
        Arm *inst = *pos; // 取出指令
        if (inst->dead)   // 无效指令
...
bool Window::match()
{
    if (cont.size() != size) // 窗口初始化失败，匹配一定失败
        return false;
    if (size == 1) // 单指令匹配
    {
        return match_single();
...
bool Window::match_double()
{
    ...
    bool flag = true;
    if (op1 == "mov" &&
        ((op2 == "add" || op2 == "sub" || op2 == "rsb") || op2 == "mul" && Plat::isReg(a11)) &&
        rs1 == a22) // mov合并到参数2
    {
        /**
         * mov r1, r2
         * add r3, r4, r1    ====>  add r3, r4, r2
         */
        inst1.setDead();
        inst2.replace(op2, rs2, a21, a11);
             ...
             ((op2 == "add" || op2 == "sub" || op2 == "rsb") || op2 == "mul" && Plat::isReg(a11)) &&
             rs1 == a21 &&
             Plat::isReg(a11)) // mov合并到参数1
    {
        /**
         * mov r1, r2
         * add r3, r1, r5    ====>  add r3, r2, r5
         */
        inst1.setDead();
        inst2.replace(op2, rs2, a11, a22);
    }
    else if (op2 == "mov" &&
             ((op1 == "add" || op1 == "sub" || op1 == "rsb") && rs1 == a21 ||
              op1 == "mul" && rs1 == a21 && rs2 != a11 && rs2 != a12))
    { // 合并mov到结果
        /**
         * add r1, r2, r3
         * mov r4, r1       ====>  add r1, r2, r3
         */
        inst1.setDead();
    ...
    else if ((op2 == "ldr" || op2 == "ldrb" || op2 == "str" || op2 == "strb") &&
             op1 == "mov" &&
             a21 == "[" + rs1 + "]" &&
             Plat::isReg(a11)) // mov合并到[r8]内存
    {
        /**
         * mov r1, r2
         * ldr r3, [r1]    ====>  ldr r3, [r2]
             ...
             rs1 == a21) // mov cmp合并比较数
    {
        ...
        inst2.replace("cmp", rs2, a11);
             ...
             op2 == "cmp" &&
             rs1 == rs2 && Plat::isReg(a11)) // mov cmp合并被比较数
    {
        /**
         * mov r1, r2
         * cmp r1, r4     ====>  cmp r2, r4
    ...
    else if (op1 == "mov" && op2 == "stmfd" &&
             Plat::isReg(a11) && a21 == "{" + rs1 + "}") // mov入栈合并
    {
        /**
         * mov r1, r2
         * stmfd sp, {r1}  ====>  stmfd sp, {r2}
         */
        inst1.setDead();
        inst2.replace(op2, rs2, "{" + a11 + "}");
    }
...
bool Window::match_triple()
{
    ...
    if ((op1 == "str" && op3 == "ldr" ||
         op1 == "strb" && op3 == "ldrb") &&
        op2 == "" && rs1 == rs3 && a11 == a31) // 重复的str ldr
    {
        /**
         * str r1, [r2]
         * nop                    str r1, [r2]
             ...
             op3 == "mov" && rs1 == rs2 && rs2 == a31) // 比较结果合并mov
    {
        /**
         * moveq r1, r2
         * movne r1, r4            moveq r5, r2
         * mov r5, r1      ====>   movne r5, r4
         */
...
{
    while (__filter(1) || __filter(2) || __filter(3))
